...
struct Teacher
{
  Teacher()
  {}
  Teacher(int id,string name,string position)
  {
    _id=id;
    _name=name;
    _position=position;
...
public:
  ...
  bool SelectAll()
  {
    ...
    for(int i=0;i<rows;i++)
    {
      ...
      printf("序号:%d 书名:%s 作者:%s 出版社:%s 类型:%s 位置:%s",atoi(row[0]),row[1],row[2],row[3],row[4],row[5]);
    }
  ...
  bool Borrow(string* x)
  {

    unique_ptr<char> sql(new char[1024]);
    std::string name="已借阅";
    sprintf(sql.get(),"update BookTable set status='%s'",name.c_str());
    int ret=mysql_query(mysql_,sql.get());
    if(ret!=0)
    {
      printf("借阅失败!%s\n",mysql_error(mysql_));
  ...
  {
    char sql[1024]={0};
    sprintf(sql,"delete from BookTable where name='%s'",value->_name.c_str());
    int ret=mysql_query(mysql_,sql);
    if(ret!=0)
    {
      printf("删除失败!%s\n",mysql_error(mysql_));
